...
for epoch in range(num_epochs):
    ...
    if valid_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), 'attention_best_model_checkpoint.pth')


# 모델 로드
model.load_state_dict(torch.load('attention_best_model_checkpoint.pth'))
...
def decode_sequence(input_seq, model, src_vocab_size, tar_vocab_size, max_output_len, int_to_src_token, int_to_tar_token):
    ...
    for _ in range(max_output_len):
        ...
        # 각 시점의 단어(정수)는 deoced_tokens 에 누적하였다가 최종 번역 시퀀스로 리턴
        decoded_tokens.append(output_token)

        # 현재 시점의 예측은 다음 시점의 입력으로 사용
